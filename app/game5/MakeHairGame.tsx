...
export default function MakeHairGame() {
  ...
  const handleClearAndRetry = async () => {
    setIsClearing(true);
    try {
      await clearGodotPayload('makehair');
      setLaunchError(null);
      setRetryNonce((v) => v + 1);
    } catch (error) {
      ...
      if (Platform.OS === 'android') {
        try {
          ...
          const launchStrategies: Array<{ className: string; extra?: Record<string, any> }> = [
            {
              className: 'expo.modules.godotview.MakeHairGodotActivity',
              extra: { ritmo_launch_mode: 'make_hair' },
            },
            {
              className: 'expo.modules.godotview.MakeHairGodotActivityLauncher',
              extra: { ritmo_launch_mode: 'make_hair' },
            },
          ];
          ...
          for (const strategy of launchStrategies) {
            ...
            if (startupFailed || abnormalCanceled) {
              console.error('Make Hair startup failed attempt', {
          ...
          if (!launchSucceeded) {
            ...
            setLaunchError(
              launchErrorText
                ? `Make Hair game failed to start (${launchErrorText}). Tap Retry to try again.`
                : 'Make Hair game failed to start. Tap Retry to try again.',
            );
            return;
          }
          ...
          if (finalStartupFailed || abnormalCanceled) {
            ...
            console.error('Make Hair startup failed final result', { finalCode, startupError, finalExtra });
            setLaunchError(
              startupError
                ? `Make Hair game failed to start (${startupError}). Tap Retry to try again.`
                : 'Make Hair game failed to start. Tap Retry to try again.',
            );
            return;
          }
          ...
          if (completedFromDurationFallback) {
            console.warn('Make Hair completion fallback applied from long session duration', {
              finalCode,
              finalElapsedMs,
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.messageContainer}>
          <Text style={styles.messageText}>Make Hair Game is only available on Android</Text>
        </View>
      </View>
    );
  }
