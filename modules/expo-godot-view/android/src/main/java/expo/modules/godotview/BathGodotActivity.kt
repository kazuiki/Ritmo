...
/**
 * Dedicated Godot host for Bath game payload.
 * Routes to bath-specific launch mode and uses base payload.
 */
class BathGodotActivity : GodotActivity() {
    companion object {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val outDir = File(filesDir, "godot-bath")
        ...
        if (!bathPayloadReady) Log.e(TAG, "Bath payload not ready: $startupFailureReason")
    ...
    override fun getCommandLine(): MutableList<String> {
        ...
        if (preferredPack == null || !projectBinaryFile.exists() || projectBinaryFile.length() <= 0L) {
            startupFailureReason = "bath_cmdline:missing_runtime_files"
    ...
    private fun clearBathRuntimePayload() {
        try {
            File(filesDir, "godot-bath").deleteRecursively()

            val knownRoots = listKnownUserDataDirs()
            for (root in knownRoots) {
                File(root, "godot-bath").deleteRecursively()
            }
    ...
    private fun prepareBathProjectPath() {
        try {
            ...
            val outBathFullPack = File(outDir, "bath_full.pck")
            ...
            val hasValidPack =
                ...
                (outBathFullPack.exists() && outBathFullPack.length() > 0L) ||
                (outSparsePack.exists() && outSparsePack.length() > 0L)
            val hasProjectBinary = outProjectBinary.exists() && outProjectBinary.length() > 0L

            if (markerValue != BATH_ASSETS_MARKER_VERSION || !hasValidPack || !hasProjectBinary) {
                if (outDir.exists()) outDir.deleteRecursively()
                outDir.mkdirs()

                copyBathAssets(outDir)
                if (!outFullMainPack.exists() || outFullMainPack.length() <= 0L) {
                    if (outBathFullPack.exists() && outBathFullPack.length() > 0L) {
                        outBathFullPack.copyTo(outFullMainPack, overwrite = true)
                    }
                }
                readyMarker.writeText(BATH_ASSETS_MARKER_VERSION)
            }

            mirrorBathAssetsToUserDataDirs(outDir)
        } catch (e: Exception) {
            startupFailureReason = "bath_prepare_exception:${e.javaClass.simpleName}:${e.message ?: "unknown"}"
            Log.e(TAG, "Failed preparing bath payload", e)
        }
    }

    private fun resolvePreferredBathPack(outDir: File): File? {
        return File(outDir, "full_main.pck").takeIf { it.exists() && it.length() > 0L }
            ?: File(outDir, "assets.sparsepck").takeIf { it.exists() && it.length() > 0L }
            ?: File(outDir, "bath_full.pck").takeIf { it.exists() && it.length() > 0L }
    }

    private fun hasPackagedBathAssets(): Boolean {
        return assetExists("bathgame/assets.sparsepck") && assetExists("bathgame/project.binary")
    }
    ...
    private fun copySingleAssetFile(assetPath: String, outputFile: File) {
        try {
            ...
            if (assetPath.endsWith("project.binary") ||
                ...
                assetPath.endsWith("bath_full.pck")
            ) {
                startupFailureReason = "bath_copy_failed:${assetPath}:${e.message ?: "io_error"}"
                Log.e(TAG, "Critical bath asset copy failed for $assetPath", e)
            }
        }
    }
    ...
    private fun mirrorBathAssetsToUserDataDirs(sourceDir: File) {
        val knownRoots = listKnownUserDataDirs()
        val preferredRoot =
