...
export const MediaTimeLimitService = {
    ...
    try {
      ...
      try {
        ...
        if (error && !isNetworkError(error)) {
          console.warn('Error saving time limit to Supabase:', error);
        }
      } catch (err) {
        if (!isNetworkError(err)) {
          console.warn('Error saving time limit to Supabase:', err);
        }
      }
    } catch (error) {
  ...
  async getTimeLimit(): Promise<MediaTimeLimit | null> {
    try {
      ...
      // Try to get from Supabase if not in local storage
      try {
        ...
        if (!isNetworkError(err)) {
          console.warn('Error fetching time limit from Supabase:', err);
        }
      }
